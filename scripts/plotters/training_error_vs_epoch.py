...
if __name__ == "__main__":
    ...
    max_epochs = max(data['Training Error'].apply(len))  # subset['Training Error'].apply(len))
    all_mse = []

    # Collect all MSE values for each epoch
    for mse_list in data['Training Error']:  # subset['Training Error']:
    ...
    # Save the plot
    plt.savefig(
        f"task1_linear_activation_training_error_vs_epoch.png",  # {varying_hyperparam.lower().replace(" ", "_")}_determined_training_error_vs_epoch_plot.png",
